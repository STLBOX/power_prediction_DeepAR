    ...
    * etc.
    ...
    def __init__(
        ...
        if min_encoder_length is None:
            min_encoder_length = max_encoder_length  # 72
        ...
        if min_prediction_length is None:
            min_prediction_length = max_prediction_length  # 24
        ...
        self.add_relative_time_idx = add_relative_time_idx   # for each sample:  -encoder_length to prediction_length

        # set automatic defaults
        if isinstance(randomize_length, bool):  # 样本随机长度时使用 min max
        ...
        if min_prediction_idx is None:
            min_prediction_idx = data[self.time_idx].min()  # minimum ``time_idx`` from where to start predictions.
        ...
        self.scalers = {} if len(scalers) == 0 else scalers  # Defaults effectively to sklearn's ``LabelEncoder()``
        ...
        data = data.sort_values(self.group_ids + [self.time_idx])  # 依据group和time_idx重新排序数据
        ...
        # create index  创建子序列，每一个包含 encoder_range 72， predict_range24  这个处理很重要
            ...
            for target in self.target_names:
                if data[target].dtype.kind != "f":  # category
                    ...
                    if self.max_encoder_length > 20 and self.min_encoder_length > 1:
                        normalizers.append(EncoderNormalizer(transformation=transformer))  # normalizers记录目标trans方法
                    else:
                        normalizers.append(GroupNormalizer(transformation=transformer))
            if self.multi_target:
                self.target_normalizer = MultiNormalizer(normalizers)
            else:
                self.target_normalizer = normalizers[0]   # 单目标EncoderNormalizer类
    ...
    def _validate_data(self, data: pd.DataFrame):
        ...
        for name in self.flat_categoricals:  # categories变量都是scr类型
    ...
    def _preprocess_data(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # encode group ids - this encoding 为_group_id_Month重新增加一列变量,采用Nan_label
    ...
    def _construct_index(self, data: pd.DataFrame, predict_mode: bool) -> pd.DataFrame:
        ...
        if (df_index["time_diff_to_next"] != 1).any():  # 只有有一个
            assert (
