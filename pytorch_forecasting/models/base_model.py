...
class BaseModel(InitialParameterRepresenterMixIn, LightningModule, TupleOutputMixIn):
            ...
            for idx in log_indices:
                ...
                if isinstance(fig, (list, tuple)):
                    for idx, f in enumerate(fig):
                        add_fig.add_figure(    # self.environment.
    ...
    def plot_prediction(
        ...
        for y_raw, y_hat, y_quantile, encoder_target, decoder_target in zip(
            ...
            y_all = torch.cat([encoder_target[idx], decoder_target[idx]])  # 所有真实值
            max_encoder_length = x["encoder_lengths"].max()  # encoder长度
            y = torch.cat(  # 和y_all相同，本任务
                (
            ...
            y_hat = y_hat.detach().cpu()[idx, : x["decoder_lengths"][idx]]  # 预测的y
            y_quantile = y_quantile.detach().cpu()[idx, : x["decoder_lengths"][idx]]   # (24, 7)
            y_raw = y_raw.detach().cpu()[idx, : x["decoder_lengths"][idx]]   # (24, 100)
