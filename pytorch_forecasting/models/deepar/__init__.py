...
class DeepAR(AutoRegressiveBaseModelWithCovariates):
    ...
    def from_dataset(
        ...
        **kwargs,
        ...
        if dataset.multi_target:
            new_kwargs.setdefault("loss", MultiLoss([NormalDistributionLoss()] * len(dataset.target_names)))  # 设置默认loss
    ...
    def forward(self, x: Dict[str, torch.Tensor], n_samples: int = None) -> Dict[str, torch.Tensor]:
        ...
        input_vector = self.construct_input_vector(
            x["decoder_cat"],
            x["decoder_cont"],
            one_off_target=x["encoder_cont"][:, -1, self.target_positions],)
        ...
        real_sapce = (output[..., 2] * output[..., 1] + output[..., 0] - x['decoder_target']).abs()
    ...
    def plot_prediction_horizon(
        ...
        for y_raw, y_hat, y_quantile, encoder_target, decoder_target in zip(
            ...
            # plot y_pred
            plotter(xs, y_hat, label="horizon={horizon}", c=pred_color, linewidth=1.5)
